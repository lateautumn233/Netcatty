...
test("readRemoteHistory probes ET sessions and parses the detected shell", async () => {
  ...
  // The command detects the login shell and branches per shell, but is forced
  // through POSIX sh so fish login shells do not parse the script.
  ...
  const api = makeApi(
    ...
    },
    {
      ensureMoshStatsConnection: async (s, id) => {
        ...
        assert.equal(s, session);
        assert.equal(id, "et-1");
        // The real helper stores the companion on moshStatsConn (NOT conn) so
        // it stays invisible to other bridges.
...
test("readRemoteHistory reports pending while the Mosh handshake is still in progress", async () => {
  // No conn and no moshStatsAuth yet — the companion can't be established.
