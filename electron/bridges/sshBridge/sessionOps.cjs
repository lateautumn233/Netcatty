...
function createSessionOpsApi(ctx) {
  with (ctx) {
    ...
    /**
     * Read the focused session's remote shell history. Detects the remote
     * login shell and tails only the matching history file
     * (~/.bash_history / ~/.zsh_history / fish_history); when the shell can't
     * be determined it falls back to reading bash+zsh+fish together. Runs on a
     * separate exec channel (or system ssh for ET) so the interactive shell is
     * untouched, and suppresses stderr so missing files degrade to empty.
     *
     * Returns { success: true, shell, bash, zsh, fish } — unread sections are
     * empty strings; the renderer parses each section per `shell`.
     */
